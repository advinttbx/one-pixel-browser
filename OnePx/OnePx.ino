...
// warm and cold temperature values (in fahrenheit, change based on preference)
const float warm_temp = 75;
const float cold_temp = 50;
...
// API key
const String key = "2e0512547f1f6868d2a29d619ca0fdf6";
  ...
  if(WiFi.status() ==  WL_CONNECTED) {
    ...
    if(httpCode > 0){
      ...
      if (error) {
        ...
        float temp_F = ((float)(jsonDoc["main"]["temp"]) - 273.15) * 9.0/5.0 + 32;
...
void color_from_temp(float temp) {
  ...
  // set pixel color to (0,0,0) initially
  strip.clear();
  strip.show();
